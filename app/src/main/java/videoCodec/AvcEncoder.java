...
//        Log.d("colorFormat","colorFormat:"+colorFormat);
//        int colorFormat =    selectColorFormat(selectSupportCodec(mime);
        try {
...
//            outStream.write(byteH264Head);
    ...
    public static void saveByteBufferToFile(String filePath,  ByteBuffer buffer ) {
//        File file = new File(filePath);
        byte[] byteH264Head = new byte[]{0,0,0,1};
//        BufferedOutputStream outStream = null;
        FileChannel fc = null;
...
//            outStream = new BufferedOutputStream(new FileOutputStream(file));
            System.out.println("save size:"+buffer.capacity());
//            outStream.write(byteH264Head);
            fc.write(buffer);
//            fc.flush();
        }  catch (Exception e) {
...
//        YUV420spRotate90Anticlockwise(rotateYuv420, yuv420, m_width, m_height);
        //YUV420spRotate180(rotateYuv420, yuv420, m_width, m_height);
        //YUV420spRotate270(rotateYuv420, yuv420, m_width, m_height);
        try {
            @SuppressWarnings("deprecation")
...
//                    }
                    int iWplusH = m_width * m_height;
                    inputBuffer.put(input,0,iWplusH);
                    inputBuffer.put(input,iWplusH+(iWplusH>>2),iWplusH>>2);
                    inputBuffer.put(input,iWplusH,iWplusH>>2);
                }else{
            ...
            int iBufferInfoSize = 0;
//            int iBufferInfoMinfoSize = 0;
            int outputBufferIndex = mediaCodec.dequeueOutputBuffer(bufferInfo, 0);

            while (outputBufferIndex >= 0) {
                ByteBuffer outputBuffer = outputBuffers[outputBufferIndex];
//                saveByteBufferToFile("/sdcard/camera_rk3288.h264", outputBuffer );
                iBufferInfoSize = bufferInfo.size;
                if(m_info!=null){
                    if( ( outputBuffer.get(4)&0x7) == 0x5) {
//                        iBufferInfoMinfoSize = m_info.length;
                        System.arraycopy(m_info, 0, output, 0, m_info.length);
                        pos += m_info.length;
                    }
//                    else iBufferInfoMinfoSize = 0;
                }
//                byte[] outData = new byte[bufferInfo.size+iBufferInfoMinfoSize];

                outputBuffer.get(output,pos,iBufferInfoSize);
                pos += iBufferInfoSize;

//                saveBytesToFile("/sdcard/camera.h264", outData);

                if (m_info != null) {

//                    System.arraycopy(outData, 0, byteH264Head, 0, byteH264Head.length);
//                    System.arraycopy(outData, 0, output, pos, outData.length);
//                    pos += outData.length;

                } else {
                    //保存pps sps 只有开始时 第一个帧里有， 保存起来后面用
                    ByteBuffer spsPpsBuffer = ByteBuffer.wrap(output);
                    if (spsPpsBuffer.getInt() == 0x00000001) {
                        m_info = new byte[iBufferInfoSize];
                        System.arraycopy(output, 0, m_info, 0, iBufferInfoSize);
                ...
                break;
//                outputBufferIndex = mediaCodec.dequeueOutputBuffer(bufferInfo, 0);
//                Log.d("AvcEncoder","while +2.outputBufferIndex:"+outputBufferIndex);
...
//                pos += m_info.length;
//                Log.d("AvcEncoder","while +2 pps sps.outputBufferIndex:");
//            }
